...
aws s3 cp s3://kontakt-telegraf-config/build-test/env . --source-region us-east-1
source ./env

API_KEY=$(curl -s -X POST "${API_URL}/manager/impersonate" \
...
echo "
{
  ...
  \"containerDefinitions\": [
    {
      \"image\": \"712852996757.dkr.ecr.us-east-1.amazonaws.com/telegraf-deployment:latest\",
...
aws ecs create-service --cluster "test-ecs-cluster" --service-name "$LABEL" --region $REGION --task-definition "$LABEL" --desired-count 1 --placement-strategy type=spread,field=instanceId
